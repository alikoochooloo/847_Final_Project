...
def plot_result(data_name_array, direct=False, k=5):
    ...
    max_to_hundred = int(math.ceil(max(max_list) / 100.0)) * 50
    
    ax.legend(handles=plt_agent_sweeps, fontsize = 13)
    ax.set_title("Learning Curve", fontsize = 15)
    ax.set_xlabel('Episodes', fontsize = 14)
    ax.set_ylabel("Reward during the episode", rotation=90, labelpad=0, fontsize = 14)
    ax.set_ylim([-1, max_to_hundred])
    plt.show()     
