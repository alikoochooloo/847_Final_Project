...
class Snake(gym.Env):
    ...
    def __init__(self, human=False, env_info={'state_space':None}):
        ...
        self.score.write(f"Total: {self.total}   \nHighest: {self.maximum}   \nDeaths: {self.deaths}", align='center', font=('Courier', 14, 'normal'))
    ...
    def close_wall(self):
        if abs(self.snake.ycor()) == 200 or abs(self.snake.xcor()) == 200:
            return True
    ...
    def get_state(self):
        ...
        state = [int(self.snake.y < self.apple.y), int(self.snake.x < self.apple.x), int(self.snake.y > self.apple.y), int(self.snake.x > self.apple.x), \
                ...
                int(self.snake.direction == 'up'), int(self.snake.direction == 'right'), int(self.snake.direction == 'down'), int(self.snake.direction == 'left')]
        '''
        st = [int(self.snake.y < self.apple.y), int(self.snake.x < self.apple.x), int(self.snake.y > self.apple.y), int(self.snake.x > self.apple.x), \
                int(wall_up or body_up), int(wall_right or body_right), int(wall_down or body_down), int(wall_left or body_left), \
                int(self.snake.direction == 'up'), int(self.snake.direction == 'right'), int(self.snake.direction == 'down'), int(self.snake.direction == 'left')]
        
        state = [[0]*HEIGHT for _ in range(WIDTH)]
        state[int(self.apple.x+9)][int(-self.apple.y+9)] = 10

        for b in  self.snake_body:
            b.x = b.xcor()/HEIGHT
            b.y = b.ycor()/HEIGHT
            # print('body',int(b.x+9),int(-b.y+9))
            if not (int(b.x+9)>19 or int(-b.y+9)>19):
                state[int(b.x+9)][int(-b.y+9)] = 1
                # return state
            # state[int(b.x+9)][int(-b.y+9)] = 1
        if not (int(self.snake.x+9)>19 or int(-self.snake.y+9)>19):
            state[int(self.snake.x+9)][int(-self.snake.y+9)] = 2
            # return state
        # print(int(self.snake.x+9),int(-self.snake.y+9))
        # state[int(self.snake.x+9)][int(-self.snake.y+9)] = 2

        state = np.reshape(state,(1,HEIGHT*WIDTH)).tolist()[0]
