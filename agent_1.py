...
def add_body(env,state):
    state.append(env.snake.xcor()/20)
    state.append(env.snake.ycor()/20)
    # state[0].append(env.body.ycor/20)
    # print(st)
    l = len(env.snake_body)
    
    for b in range(l):
        state.append(env.snake_body[b].ycor()/20)
        state.append(env.snake_body[b].ycor()/20)
        # state[0].append(b.xcor/20)
        # state[0].append(b.ycor/20)
    if l < 20:
        for i in range(20-l):
            state.append(0)
            state.append(0)
...
# writing the data into the file
    ...
    #     write.writerows([sum_of_rewards])
    results[params['name']] = sum_of_rewards
    
    plot_result(results, direct=True, k=5)
